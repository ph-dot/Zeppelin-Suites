...
<aside class="sidebar fixed left-0 top-0 h-full border-r border-slate-100/80 flex flex-col z-50 md:z-40 shadow-2xl md:shadow-none" id="sidebar">
  ...
  <nav class="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto overflow-x-hidden">
    ...
    <a href="../adminPages/reservation.php" data-tooltip="Lease Management" class="sidebar-link active flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-500">
...
<div class="main-wrapper h-screen flex flex-col" id="mainWrapper">
  <header class="glass-header border-b border-slate-100/80 px-4 md:px-6 py-3.5 flex items-center gap-4 shrink-0 z-30">
    ...
    <div class="flex items-center gap-2 text-sm text-slate-500">
      <a href="reservation.php" class="hover:text-slate-900 transition-colors font-medium">Lease Management</a>
      <span>/</span>
      ...
      <div class="bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-sm">
        ...
        <div id="tabContent-lease" class="tab-panel space-y-6">
          ...
          <?php if ($cancellationStatusLower === 'requested'): ?>
            <section id="cancellationRequestSection" class="mt-6 bg-red-50 border border-red-200 rounded-2xl p-6 shadow-sm">
        ...
        <section class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-sm">
          ...
          <?php else: ?>
            ...
            <div class="mt-5 rounded-xl border border-blue-200 bg-blue-50/90 px-5 py-4 flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="text-xs font-bold uppercase tracking-wider text-blue-700">Awaiting Unit Owner Verification</p>
                <p class="text-sm font-medium text-blue-950 mt-0.5">
                  Downpayments are sent directly to the assigned unit owner's GCash QR code. The assigned unit owner (<strong><?= e($res['owner_name'] ?: 'Unit Owner') ?></strong>) is responsible for checking their account and verifying this payment in their portal.
                </p>
                <?php if (!empty($res['admin_payment_remarks'])): ?>
                  <p class="text-xs text-blue-800 mt-1.5 italic">Current note: <?= e($res['admin_payment_remarks']) ?></p>
                <?php endif; ?>
              </div>
            </div>
            ...
            <div class="mt-4 pt-3 border-t border-slate-200">
              <details class="text-xs text-slate-500">
                <summary class="cursor-pointer font-semibold text-slate-600 hover:text-slate-900 inline-flex items-center gap-1.5 py-1">
...
<script>
  ...
  async function submitLeaseSigningAction() {
    ...
    try {
      ...
      const res = await fetch('ActionsAP/completeLeaseSigning.php', {
        method: 'POST',
        body: formData
      });
  ...
  <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100">
    ...
    <div class="p-6 space-y-4">
      ...
      <div>
        ...
        <textarea id="inHousePaymentRemarks" rows="2" placeholder="e.g. Cash downpayment received in full at the office." class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 resize-none focus:outline-none focus:border-slate-900"></textarea>
      </div>
    </div>
