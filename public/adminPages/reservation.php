...
<aside class="sidebar fixed left-0 top-0 h-full border-r border-slate-100/80 flex flex-col z-50 md:z-40 shadow-2xl md:shadow-none" id="sidebar">
  ...
  <nav class="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto overflow-x-hidden">
    ...
    <a href="../adminPages/reservation.php" data-tooltip="Reservation" class="sidebar-link active flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium">
          ...
          <table class="w-full text-sm" id="resTable">
            <thead>
              <tr class="border-b border-slate-100 bg-slate-50/60">
                ...
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Full Name</th>
                ...
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Resident Type</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Res. Type</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Unit</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Fee</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Duration</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Move-in</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Status</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Txn</th>
...
<div class="modal-backdrop fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4" id="editModalBackdrop" onclick="handleEditBackdropClick(event)">
  <div class="modal-card bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden">
    <div class="bg-slate-900 px-6 py-4 flex items-center justify-between">
      <div>
        <h2 class="text-base font-bold text-white">Reservation Details</h2>
        <p class="text-xs text-slate-400 mt-0.5" id="editModalMeta">—</p>
      </div>
      <button onclick="closeEditModal()" class="btn-press p-1.5 rounded-lg hover:bg-white/10 transition-colors active:scale-95">
    ...
    <div class="p-6 overflow-y-auto max-h-[70vh]">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input id="ef_name" type="text" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Email</label>
          <input id="ef_email" type="email" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Contact</label>
          <input id="ef_contact" type="text" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Unit</label>
          <input id="ef_unit" type="text" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Resident Type</label>
          <select id="ef_restype" class="zep-select w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
            <option>Tenant</option>
            <option>Unit Owner</option>
          </select>
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Reservation Type</label>
          <select id="ef_type" class="zep-select w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
            <option>Reservation</option>
            <option>Pre-selling</option>
          </select>
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Reservation Fee</label>
          <input id="ef_fee" type="text" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Lease Duration</label>
          <input id="ef_lease" type="text" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Move-in Date</label>
          <input id="ef_movein" type="date" class="zep-input w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Status</label>
          <select id="ef_status" class="zep-select w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Transaction</label>
          <select id="ef_txn" class="zep-select w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 transition-all">
            <option>Paid</option>
    ...
    <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/60">
      <button onclick="closeEditModal()" class="btn-press px-5 py-2 text-sm font-semibold text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100 transition-all active:scale-95">Cancel</button>
      <button onclick="saveEdit()" class="btn-press bg-slate-900 hover:bg-slate-700 text-white text-sm font-semibold px-6 py-2 rounded-xl transition-all active:scale-95">Save Changes</button>
    </div>
  </div>
</div>
...
function doLogout() {
  ...
  function openEditModal(row) {
    editRow = row;
    document.getElementById('editModalMeta').textContent = `Reservation #${getCellText(row,0)}`;
    document.getElementById('ef_name').value = getCellText(row, 1);
    document.getElementById('ef_email').value = getCellText(row, 2);
    document.getElementById('ef_contact').value = getCellText(row, 3);
    document.getElementById('ef_restype').value = getCellText(row, 4);
    document.getElementById('ef_type').value = getCellText(row, 5);
    document.getElementById('ef_unit').value = getCellText(row, 6);
    document.getElementById('ef_fee').value = getCellText(row, 7);
    document.getElementById('ef_lease').value = getCellText(row, 8);
    document.getElementById('ef_movein').value = getCellText(row, 9);
    // set status select
    const statusTxt = getCellText(row, 10);
    const ss = document.getElementById('ef_status');
    for (let o of ss.options) { if (o.value === statusTxt) { ss.value = statusTxt; break; } }
    const txnTxt = getCellText(row, 11);
    const ts = document.getElementById('ef_txn');
    for (let o of ts.options) { if (o.value === txnTxt) { ts.value = txnTxt; break; } }
    document.getElementById('editModalBackdrop').classList.add('open');
  }

  function closeEditModal() { document.getElementById('editModalBackdrop').classList.remove('open'); }
  function handleEditBackdropClick(e) { if (e.target === document.getElementById('editModalBackdrop')) closeEditModal(); }
  ...
  function setPill(cell, value, map) {
    const span = cell.querySelector('span');
    if (!span) return;
    span.className = `text-xs font-semibold px-2.5 py-1 rounded-full border ${map[value] || 'bg-slate-100 text-slate-500 border-slate-200'}`;
    span.textContent = value;
  }

  function saveEdit() {
    if (!editRow) return;
    editRow.cells[1].querySelector('.res-name') ? editRow.cells[1].querySelector('.res-name').textContent = document.getElementById('ef_name').value : null;
    editRow.cells[1].textContent = document.getElementById('ef_name').value;
    editRow.cells[1].className = 'px-4 py-3.5 font-semibold res-name text-slate-800 whitespace-nowrap';
    editRow.cells[2].textContent = document.getElementById('ef_email').value;
    editRow.cells[3].textContent = document.getElementById('ef_contact').value;
    editRow.cells[4].textContent = document.getElementById('ef_restype').value;
    editRow.cells[5].textContent = document.getElementById('ef_type').value;
    editRow.cells[6].textContent = document.getElementById('ef_unit').value;
    editRow.cells[7].textContent = document.getElementById('ef_fee').value;
    editRow.cells[8].textContent = document.getElementById('ef_lease').value;
    editRow.cells[9].textContent = document.getElementById('ef_movein').value;
    setPill(editRow.cells[10], document.getElementById('ef_status').value, statusMap);
    setPill(editRow.cells[11], document.getElementById('ef_txn').value, txnMap);
    closeEditModal();
  }
</script>
</body>
