...
<div class="main-wrapper h-screen flex flex-col" id="mainWrapper">
      ...
      <div class="relative" id="profileWrapper">
        ...
        <div class="profile-dropdown absolute right-0 top-full mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg py-1 z-50 hidden" id="profileDropdown">
          <div class="px-3.5 py-2 border-b border-slate-100">
            <p class="text-xs font-bold text-slate-800"><?= htmlspecialchars($user['full_name'] ?? 'Unit Owner') ?></p>
            <p class="text-[11px] text-slate-400 font-mono truncate"><?= htmlspecialchars($user['email'] ?? '') ?></p>
          </div>
          <button onclick="confirmLogout()" class="w-full text-left flex items-center gap-2 px-3.5 py-2 text-xs text-red-600 hover:bg-red-50 transition-colors">
  ...
  <div id="logoutModal" class="fixed inset-0 bg-black/50 z-[999] hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm border border-slate-100 shadow-2xl">
      <h3 class="text-base font-bold text-slate-900 mb-1.5">Sign out?</h3>
      <p class="text-xs text-slate-600 mb-5">Are you sure you want to end your current session?</p>
      <div class="flex gap-2.5 justify-end">
        <button onclick="hideLogoutModal()" class="px-4 py-2 text-xs font-semibold text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all">Cancel</button>
        <button onclick="doLogout()" class="px-4 py-2 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-xl transition-all shadow-xs">Logout</button>
      </div>
    </div>
  </div>
