<?php
/**
 * saveOwnerBlockedDate.php (Unit Owner)
 * Allows a unit owner to block dates for maintenance or unavailability
 * on units they own, with custom remarks.
 */
...
// 1. Verify unit belongs to this logged-in owner
$stmtUnit = $conn->prepare("SELECT unit_id, unit_number, unit_type FROM units_table WHERE unit_id = ? AND unit_owner_id = ? LIMIT 1");
if (!$stmtUnit) {
...
if (!$unit) {
    echo json_encode(['success' => false, 'message' => 'Unit not found or you do not have permission to manage this unit.']);
    exit;
}
...
$stmtInsert->bind_param('issssi', $unitId, $startDate, $endDate, $blockType, $remarks, $ownerId);
